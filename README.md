...
services:
  ...
  backend:
    ...
    ports:
      - "4000:4000"
    environment:
      DATABASE_URL: "postgresql://postgres:password123@postgres:5432/dashboard_db"
      JWT_ACCESS_SECRET: "dev-secret-key-minimum-32-characters-long"
      JWT_REFRESH_SECRET: "dev-secret-key-minimum-32-characters-lon"
      CORS_ORIGIN: "http://localhost:3000"
    depends_on:
      postgres:
...
3. **Seed database (in running backend container):**
```bash
